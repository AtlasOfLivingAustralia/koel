...
#' directories.
#'
#' @param alerts_data A data.frame with eight columns and a row for every taxon
#'   occurrence record. May be produced by `download_occurrences()`.
#' @param cache_path A string containing the path to the temporary cache folder
...
#' @param template_path A single string containing the path to the R markdown
...
#' @param email_list A data.frame with at least two columns, the first named
...
#' @param email_subject An optional string of the subject of the email. If not
#'   provided, defaults to "ALA Biosecurity Alerts".
#' @param email_send A string providing the email address from which the alerts
#'   are to be sent. Defaults to `NA`.
#' @param email_password A string providing the password for the provided email
#'   address (`email_send` argument). Defaults to `NA`.
#' @param email_host A string providing the email server host to be fed to the
#'   {emayili} function `server()`. Defaults to `NA`.
#' @param email_port A numeric value providing the email server port to be fed
